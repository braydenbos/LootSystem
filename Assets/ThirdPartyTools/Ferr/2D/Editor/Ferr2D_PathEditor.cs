...
public class Ferr2D_PathEditor : Editor { 
...
#if UNITY_5_6_OR_NEWER
    private void CapDotMinus        (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texMinus,           aType);}
    private void CapDotMinusSelected(int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texMinusSelected,   aType);}
    private void CapDot             (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDot,             aType);}
    private void CapDotSnap         (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDotSnap,         aType);}
    private void CapDotPlus         (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDotPlus,         aType);}
    private void CapDotSelected     (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDotSelected,     aType);}
    private void CapDotSelectedSnap (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDotSelectedSnap, aType);}
    private void CapDotLeft         (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texLeft,            aType);}
    private void CapDotRight        (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texRight,           aType);}
    private void CapDotTop          (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texTop,             aType);}
    private void CapDotBottom       (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texBottom,          aType);}
    private void CapDotAuto         (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texAuto,            aType);}
    private void CapDotScale        (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texScale,           aType);}
    private void CapDotReset        (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texReset,           aType);}
	private void CapDot1            (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDot1,            aType);}
	private void CapDot2            (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDot2,            aType);}
	private void CapDot3            (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDot3,            aType);}
	private void CapDot4            (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDot4,            aType);}
	private void CapDot5            (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDot5,            aType);}
	private void CapDotN            (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, UnityEngine.EventType aType) {Ferr.EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, texDotN,            aType);}
#else
	...
	private         void OnSceneGUI    () {
		...
		if (Event.current.type == UnityEngine.EventType.Repaint && terrain != null)
		...
		if (Event.current.type == UnityEngine.EventType.ValidateCommand) {
		...
		if (GUI.changed) {
			...
			prevChanged = true;
		} else if (Event.current.type == UnityEngine.EventType.Used) {
			if (prevChanged == true) {
	...
	private void    DoHandles            (Ferr2D_Path path, GUIStyle iconStyle)
	{
		...
		if (Event.current.type == UnityEngine.EventType.KeyDown && Event.current.keyCode == KeyCode.Delete && selectedPoints.Count > 0) {
			deleteSelected = true;
			GUI.changed = true;
			Event.current.Use();
		}
