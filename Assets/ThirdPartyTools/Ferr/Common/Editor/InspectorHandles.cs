		...
		public static Vector3 FreeMove(Vector3 position, float size, Handles.CapFunction handleFunction) {
			...
			switch (current.GetTypeForControl(id)) {
				...
				case UnityEngine.EventType.Repaint: {
						...
						handleFunction(id, position2, Quaternion.identity, size, UnityEngine.EventType.Repaint);
				...
				case UnityEngine.EventType.Layout:
					Handles.matrix = Matrix4x4.identity;
					handleFunction(id, position2, Quaternion.identity, size, UnityEngine.EventType.Layout);
					Handles.matrix = matrix;
					break;
			}
			return position;
		...
		public static bool Button(Vector3 position, float size, float pickSize, Handles.CapFunction capFunction) {
			...
			switch (current.GetTypeForControl(id)) {
				...
				case UnityEngine.EventType.Repaint: {
						...
						capFunction(id, position, Quaternion.identity, size, UnityEngine.EventType.Repaint);
						Handles.color = color;
						break;
					}
				case UnityEngine.EventType.Layout:
					if (GUI.enabled) {
						capFunction(id, position, Quaternion.identity, pickSize, UnityEngine.EventType.Layout);
					}
					break;
			}
