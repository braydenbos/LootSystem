...
public class Ferr2DT_SceneOverlay {
    ...
    public static void OnGUI() {
		...
		if (Event.current.control && Event.current.type == UnityEngine.EventType.KeyUp && Event.current.keyCode == KeyCode.R) {
	        Ferr2DT_Menu.SmartSnap = !Ferr2DT_Menu.SmartSnap;
        }
		if (Event.current.control && Event.current.type == UnityEngine.EventType.KeyUp && Event.current.keyCode == KeyCode.L) {
		...
		if (Ferr2DT_Menu.ShowHelp) {
			...
			EditorGUILayout.LabelField("\u2022 CTRL to delete or reset", HelpLabelStyle);
