...
public static class TransformUtilities
{
    ...
    /// <param name="scale">Optional scale parameter that the gameObject's scale will be reverted to.<br></br> (Default: Vector3.one)</param>
    public static void Hide(this GameObject target, bool isHidden = true, Vector3 scale = new Vector3())
    {
        var localScale = scale == Vector3.zero ? Vector3.one : scale;
        if (isHidden)localScale = Vector3.zero;
        target.transform.localScale = localScale;
    }
}